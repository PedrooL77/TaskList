      ...
      <div className="BlocoTarefa">
        ...
        <div className="tarefas">
          <ul>
            ...
            <li className={item.status ? 'ItemAtivo' : 'ItemInativo'} key={index}>{item.texto} <button className="lixo" onClick={ () => statusTarefa(item.id, item.status) }>{item.status ? <i class="fa-solid fa-check"></i> : <i class="fa-solid fa-ellipsis"></i>}</button><i class="fa-solid fa-trash" onClick={ () => excluirTarefa(item.id) }></i></li>
            ))}
          </ul>
        </div>
      </div>
